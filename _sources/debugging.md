...
name: breakpoint
---
A breakpoint on line 6 in PyCharm.
```

As shown in {ref}`breakpoint`, the red dot, is the breakpoint.  Next, click on the Debug ![](images/debug.png) button.  The code will start executing until it reaches the first breakpoint which essentially 
pauses the code from executing, allowing you to see the values of the variables at that point.  In 
the example below, we can see that when the code pauses execution on line 6, `i` has the value of 4, `even` [0,5] and `odd` [1,3] in the 
